...
L = [[(lambda v, n, p, x=x: [v[0] + 1, (v[1] * (1 - x)) + n] + v[2:] if p is not None and n == p + 1 - x else [1, n] + v[2:]), (lambda v: v[:2] + v[:2] if v[0] > v[2] or (v[0] == v[2] and v[1] > v[3]) else v), [0] * 4] for x in range(2)]
